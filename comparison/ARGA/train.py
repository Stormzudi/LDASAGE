...
import pandas as pd
from util.load_data import load_data, sparse_to_tuple, mask_test_edges, preprocess_graph
from util.loss import arga_loss_function, varga_loss_function
from util.metrics import get_roc_score
from util import define_optimizer
from model.arga import GCNModelARGA
import datetime
...
class Train():
    ...
    def train_model(self, config_path):
        if os.path.exists(config_path) and (os.path.split(config_path)[1].split('.')[0] == 'config') and (
            ...
            # 加载相关数据
            adj, nodeList, nodeAll = load_data(os.path.join(data_catalog, train_file_name))
            ...
            # 定义真实值和预测值
            preds_all = np.array([0])
            labels_all = np.array([0])
            ...
            for epoch in range(epochs):
                ...
                # 评估验证集，val set
                roc_score, ap_score, aupr_score, labels, preds = get_roc_score(hidden_emb, adj_orig, val_edges, val_edges_false)
                ...
                if roc_score > best_valid_roc_score:
                    ...
                    np.save(model_path_, hidden_emb)
            ...
            # 选取 1337个 data
            import random
            sample_list = [i for i in range(len(preds_all))]
            sample_list = random.sample(sample_list, 1337)

            preds_all = preds_all[1:][sample_list]
            labels_all = labels_all[1:][sample_list]
            # save auc score
            data_ = pd.DataFrame({
                'pred': preds_all,
                'labels': labels_all
            ...
            data_.to_csv("{}/{}_ARGA_0.91.csv".format(model_path,date))

            # 评估测试集，test set
            roc_score, ap_score, aupr_score, labels, preds = get_roc_score(hidden_emb, adj_orig, test_edges, test_edges_false)
            print('test roc score: {}'.format(roc_score))
            print('test ap score: {}'.format(ap_score))
            print('test aupr score: {}'.format(aupr_score))
