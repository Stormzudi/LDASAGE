...
class GCNNet(nn.Module):
    ...
    def forward(self, adj, features):
        out = F.relu(self.gcn1(adj, features), inplace=False)  # 1061*700
        # print("out1: ",out.shape)
        out = self.gcn2(adj, out)  # 1061*500
...
class Attention_fusion(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        # reslut = torch.cat((x1, x2, x3), dim=1)  # 1,2,1061 按照列拼接
        # x:1061*768
        ...
        # print("W_out_size :", temp_node_1.size(),temp_node_2.size(),temp_node_3.size())     # d*d  128*128
        ...
        # print("h_out_size  :",node_score_1.size(),node_score_2.size(),node_score_3.size())  #1,d  1,128

        nodes_score = torch.cat((node_score_1, node_score_2, node_score_3), dim=0)  # 3,128
        # print("nodes_score: ",nodes_score.shape)

        # nodes_score = torch.cat((node_score_1,node_score_2,node_score_3),dim=1)
        # print("nodes_score: ", nodes_score.shape)
        beta = F.softmax(nodes_score, dim=0)  # 3,128
        # print(beta[3].size())
        # print("beta: ", beta[0].size(),x1[0].size())

        x_l = torch.zeros((1061, 128))
        x_d = torch.zeros((1061, 128))
        x_m = torch.zeros((1061, 128))
        for i in range(1061):
            x_l[i] = torch.mul(beta[0], x1[i])  # (1*128) * (1*128)
            x_d[i] = torch.mul(beta[1], x2[i])
            x_m[i] = torch.mul(beta[2], x3[i])

        # z_i = torch.matmul(beta, x)  # 1 * 1 * 1061
        # print("zi: ", z_i.shape)
...
class MGATE(nn.Module):
    ...
    def forward(self, adj_c, features_c, adj_l, features_l, adj_m, features_m, adj_d, features_d,
        ...
        out_c = self.gcn_complex(adj_c, features_c)  # 1061 * out_dim
        ...
        out_intra = torch.cat((out_l, out_d, out_m), dim=0)  # 1061 * out_dim

        out_inter_1 = torch.cat((out_ld[:350], out_ld[350:], out_lm[350:]), dim=0)
        out_inter_2 = torch.cat((out_lm[:350], out_md[233:], out_md[:233]), dim=0)
        out_inter = (out_inter_1 + out_inter_2) / 2  # 1061 * out_dim
        # print("intra_graph: ", out_inter_1.shape,out_inter_2.shape)

        out_cross_fusion = self.attention(out_c, out_intra, out_inter)
